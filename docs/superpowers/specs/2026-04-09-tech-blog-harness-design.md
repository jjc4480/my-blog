...
1. **디자인 레퍼런스**: `references/design-refs.md`에 벤치마킹 블로그 URL 관리. 작업 시 실제 사이트를 브라우저로 확인하고 스타일 참고.
2. **콘텐츠 톤 레퍼런스**: `content/posts/`에 쌓인 기존 포스트의 톤/구조를 분석해서 일관성 유지. 처음엔 외부 블로그 참고, 포스트가 쌓이면 자기 자신이 기준.
3. **QA 검증**: 디자인 레퍼런스와 비교해서 "템플릿 느낌" 체크. 기존 포스트와 톤 일관성 확인.
