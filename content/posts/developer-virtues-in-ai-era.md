...
프레임워크 사용법은 AI가 알려준다. 하지만 왜 그렇게 동작하는지를 모르면, 문제가 생겼을 때 손을 못 쓴다.
...
AI 출력을 무조건 신뢰하지 않는 습관이 필요하다. 동료 코드를 리뷰하듯, AI가 짠 코드도 보안·성능·정확성 관점에서 한 번 더 들여다봐야 한다. 그 한 번이 차이를 만든다.
...
이걸 해결하려는 접근이 하네스 엔지니어링이다. 말(馬)에 고삐를 채우듯, AI 에이전트에게 명확한 정체성, 도구 권한, 행동 규칙, 메모리 구조를 부여해서 예측 가능하게 만드는 것. 이 개념에 대해서는 [별도 글](/posts/harness-engineering)에서 더 자세히 다룬다.
