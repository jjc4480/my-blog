...
핵심은 간단하다. 에이전트의 **정체성, 도구 권한, 컨텍스트, 메모리, 오케스트레이션, 모듈화**를 코드처럼 선언적으로 관리하는 것.
...
AI 에이전트를 잘 쓰는 것과 그냥 쓰는 것의 차이는 결국 구조의 유무다. 코드를 그냥 짜는 것과 아키텍처를 설계해놓고 짜는 것이 다르듯, 에이전트도 마찬가지다. 하네스 엔지니어링은 에이전트를 소프트웨어처럼 다루는 접근이다. 선언적으로 정의하고, 버전 관리하고, 테스트 가능하게 만든다. 그러면 LLM이 바뀌어도, 규칙이 바뀌어도, 여러 명이 한 팀으로 돌려도 흔들리지 않는다.

## 참고 자료

- [Building effective agents (Anthropic)](https://www.anthropic.com/research/building-effective-agents): 에이전트 패턴과 설계 원칙 공식 가이드
- [Tool use with Claude (Anthropic)](https://docs.anthropic.com/en/docs/agents-and-tools/tool-use/overview): Claude의 도구 호출 모델 문서
- [A practical guide to building agents (OpenAI)](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf): 에이전트 구성 요소와 운영 관점 정리
- [Model Context Protocol 스펙](https://modelcontextprotocol.io): 에이전트에 도구/컨텍스트를 붙이는 표준 프로토콜
