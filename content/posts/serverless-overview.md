...
### A. S3에 직접 올리게 하는 업로드 패턴 (presigned URL)
...
### B. S3 이벤트로 Lambda를 깨워 이미지/영상 후처리
...
- **트래픽 없을 때 비용 0.** 사이드 프로젝트나 개인 도구엔 특히 좋다. 월 10만 요청 이하면 사실상 공짜인 플랜도 많다.
- **자동 확장.** 동시 요청이 갑자기 1000개가 들어와도 별 설정 없이 처리된다. 확장 때문에 밤에 깨는 일이 줄어든다.

### 단점

- **처음 실행이 느리다 (콜드 스타트).** 함수가 처음 호출될 때는 런타임을 띄우는 데 시간이 걸린다. Node.js는 보통 100~300ms, Java는 1~3초. 응답 속도에 민감한 API라면 문제가 된다.
