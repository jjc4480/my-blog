...
**인증(Authentication)**은 신원 확인이다. 공항에서 여권을 보여주는 것과 같다. "이 사람이 사용자 A가 맞는가?"
...
세션 기반 인증의 가장 큰 한계는 서버가 상태를 가진다는 점이다. 서버가 한 대일 때는 문제 없지만, 서버를 여러 대 띄우면 세션을 공유해야 한다. Redis 같은 외부 저장소를 세션 스토어로 쓰는 게 일반적이다.
...
- Access Token은 클라이언트 메모리(JavaScript 변수)에 저장한다. `localStorage`에 넣으면 XSS로 탈취될 수 있다.
- Refresh Token은 `HttpOnly` 쿠키에 저장한다. JavaScript에서 접근 불가하므로 XSS에 안전하다.

실무에서는 Refresh Token을 그대로 재사용하게 두면 안 된다. 재발급할 때마다 새 Refresh Token으로 교체하고, 이전 토큰은 폐기한다. 유출됐을 때 피해 범위를 줄이기 위해서다. 쿠키 기반으로 `/refresh`를 호출한다면 CSRF 방어도 같이 봐야 한다. `SameSite`만 믿지 말고 CSRF 토큰이나 origin 검증을 붙이는 편이 안전하다.
...
app.get("/callback", async (req, res) => {
  ...
  if (!stateStore.has(state as string)) return res.status(400).end();
...
예제라 PKCE와 저장소 처리는 줄였다. 운영 코드라면 `state`는 세션이나 짧은 TTL 저장소에 보관하고, 공개 클라이언트에서는 PKCE를 반드시 붙인다.
...
OIDC(OpenID Connect)는 OAuth2 위에 인증 레이어를 추가한 확장이다. Access Token 외에 **ID Token**(JWT 형식)을 추가로 발급한다.
