...
> **한 줄 요약:** 인증은 "누구인가", 인가는 "뭘 할 수 있는가". 이 두 질문에 답하는 방식이 Basic Auth에서 Passkey까지 계속 진화해왔다.

웹 서비스를 만들면 가장 먼저 부딪히는 문제가 있다. "이 요청을 보낸 사람이 누구인지 어떻게 알지?" 그리고 바로 뒤따르는 질문. "이 사람이 이 작업을 해도 되는 건지 어떻게 판단하지?"

이 두 질문이 바로 인증(Authentication)과 인가(Authorization)다. 비슷해 보이지만 완전히 다른 문제이고, 혼동하면 보안 구멍이 생긴다. 이 글에서는 두 개념의 차이부터 시작해서, 웹에서 쓰이는 주요 인증/인가 방식을 코드와 함께 살펴본다.
...
모바일 앱에서도 쿠키 관리가 번거롭다. 이런 한계를 해결하려고 나온 것이 토큰 기반 인증이다.
...
이걸 해결하는 패턴이 Access Token과 Refresh Token의 이중 구조다.
...
아직 모든 서비스에서 지원하는 건 아니지만, 구글, 애플, 마이크로소프트가 적극적으로 밀고 있다. 장기적으로 비밀번호를 대체할 가능성이 높다.
...
Keycloak 같은 전용 솔루션을 두고 앱 서버는 토큰 검증만 하는 구조가 실무에서는 훨씬 현실적이다. 인증이라는 관심사를 별도 서비스로 분리하는 것, 결국 모듈화의 문제다.
...
어떤 방식을 선택할지는 서비스의 성격에 달려있다. 전통적인 서버 렌더링 웹이라면 세션이 가장 간단하다. SPA나 모바일 앱이라면 JWT. 여러 서비스를 운영한다면 OAuth2/OIDC 기반 SSO. 그리고 이 모든 걸 직접 구현하기보다는, Keycloak이나 Auth0 같은 검증된 솔루션 위에서 시작하는 게 현실적이다.

보안은 직접 만들수록 구멍이 생기기 쉬운 영역이다. 잘 만들어진 도구를 쓰고, 그 도구가 어떻게 동작하는지 이해하는 것. 그게 가장 안전한 접근이다.

## 참고 자료

- [인증과 인가 세미나 영상](https://www.youtube.com/watch?v=iX8g4LqF8p8) - 이 글의 바탕이 된 사내 세미나 발표
