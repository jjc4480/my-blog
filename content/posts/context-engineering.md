...
[1편 하네스 엔지니어링](/blog/harness-engineering)에서는 정체성, 도구, 규칙, 메모리, 컨텍스트를 묶어 에이전트를 소프트웨어처럼 다루는 큰 그림을 다뤘다. 그 중 "컨텍스트 설계" 축만 따로 떼서 깊게 파는 글이 이번 편이다.
...
1편에서 본 하네스 엔지니어링의 다섯 축 중 하나를 풀어쓴 글이다. 나머지 축(툴 정책, 메모리 아키텍처, 멀티 에이전트 오케스트레이션)은 다음 시리즈에서 하나씩 이어갈 예정이다.
