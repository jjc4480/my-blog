...
실용적인 접근: 모놀리스로 시작하되 내부를 모듈로 잘 나눠두고, 분리가 필요한 시점에 마이크로서비스로 추출한다. 트래픽이나 조직 규모가 뒷받침되지 않는데 처음부터 MSA로 가는 건 대부분의 팀에게 과한 선택이다.
...
기술을 선택하는 건 쉽다. 그 기술이 가져오는 **트레이드오프를 팀이 감수할 준비가 되어 있는지**가 더 중요하다.
