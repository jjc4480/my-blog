...
이 글은 GraphQL을 쓰기로 결정한 팀을 위한 구현 가이드다. 아직 GraphQL이 필요한지 판단 중이라면 [GraphQL vs REST 2026](/blog/2026-04-24-graphql-vs-rest-2026)을 먼저 보라. 복수 클라이언트, 심각한 over/under-fetching, 복잡한 중첩 데이터 없이 REST로 충분한 경우가 많다.

ent, gqlgen, entgql 조합은 이 문제를 **코드 생성**으로 해결한다. ent 스키마 하나만 정의하면 나머지 코드가 자동으로 생성된다. 각 도구의 역할과 연결 방식, 그리고 Relay Connection 기반 페이지네이션까지 정리해봤다.
...
[entgql](https://pkg.go.dev/entgo.io/contrib/entgql)은 ent와 gqlgen을 연결하는 도구다. ent 스키마에 어노테이션을 붙이면:

1. ent 스키마에서 GraphQL 스키마를 자동 생성한다
2. Relay Node 인터페이스를 구현한다
3. Relay Connection (cursor-based pagination)을 자동 생성한다
4. 필드 컬렉션으로 N+1 문제를 해결한다
5. 트랜잭셔널 뮤테이션을 지원한다

한마디로, ent 스키마를 정의하면 GraphQL API 기본 구조가 자동 생성된다.
...
- **페이지가 뒤로 갈수록 느려진다.** `OFFSET 10000`이면 DB가 앞의 10000행을 읽고 버린 뒤 결과를 돌려준다
...
| 디버깅 | 없음 | 생성 코드 안에서 버그가 터지면 추적이 어렵다 |
| 커스터마이징 | 없음 | 기본 패턴에서 벗어나는 요구사항은 우회가 필요하다 |

코드 생성은 **80%의 케이스를 자동화하되, 20%의 예외를 처리하는 비용**이 존재한다. ent + gqlgen + entgql 조합은 CRUD 중심의 API에서는 생산성이 압도적이다. 하지만 복잡한 비즈니스 로직이 resolver에 많이 들어가거나, GraphQL 스키마를 세밀하게 제어해야 하는 경우에는 생성 코드 밖에서 직접 구현해야 하는 부분이 늘어난다.
...
스키마 하나가 DB부터 API까지 전 계층에 적용된다. 각 도구는 자기 영역만 책임지고, 어노테이션으로 연결된다. 모듈 간 경계가 명확하기 때문에 한 도구를 교체해도 다른 도구에 미치는 영향이 작다.
