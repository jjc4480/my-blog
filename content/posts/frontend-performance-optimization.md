...
자주 만나는 6가지를 Before/After 코드로 정리했다.
...
상태를 실제로 쓰는 컴포넌트 안으로 내리면, 그 컴포넌트만 재렌더링된다. 간단한데 효과가 크다.
### memo + useCallback 조합
...
이미지와 폰트는 페이지 로딩 속도와 화면 안정성에 직접 영향을 주는 요소다. 포맷을 바꾸고 로딩 순서를 조정하는 것만으로도 차이가 크다.
...
Skeleton과 실제 콘텐츠의 높이가 다르면 어차피 밀림이 생기니까, 크기를 맞추는 게 핵심이다.
## 5. 이벤트 핸들러
...
CSS든 Canvas든 원리는 같다. 매 프레임 CPU가 하던 무거운 작업을 GPU로 넘기거나 Worker로 분리하면, 메인 스레드가 가벼워진다.
## 체크리스트
...
다 적용할 필요는 없다. Chrome DevTools Performance 탭이나 Lighthouse로 병목을 먼저 찾고, 해당되는 것만 고치면 된다. 감으로 하는 최적화는 대체로 효과가 없다.
