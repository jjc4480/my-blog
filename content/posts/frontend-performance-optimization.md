...
이 글의 예제는 전부 React 기준이다. Svelte나 SolidJS를 쓴다면 아래 최적화 대부분이 컴파일 타임에 자동으로 처리되거나 아예 불필요하다. 리렌더링 제어, memo/useMemo/useCallback은 React 특유의 문제이지, 프론트엔드 성능 최적화의 보편적 과제가 아니다.
...
`memo`만 걸고 `useCallback`을 빠뜨리는 경우가 꽤 많은데, `memo`는 prop이 안 바뀌었을 때만 건너뛰는 거라 함수가 매번 새로 만들어지면 소용이 없다.
...
브라우저가 AVIF 되면 AVIF, 안 되면 WebP, 둘 다 안 되면 JPEG. `<picture>` 태그 하나로 끝난다.
...
주의할 점은, 첫 화면에 보이는 이미지에 `loading="lazy"`를 걸면 오히려 느려진다. 화면에 들어올 때까지 로딩을 미루니까. 첫 화면 이미지는 기본값 그대로 두고, 가능하면 `preload`까지 걸어주는 게 좋다.
...
사용자가 `/dashboard`에 접속하면 Dashboard 코드만 받는다. 나머지는 해당 페이지 갈 때 받으면 되니까. 이것만으로도 초기 번들이 40~60% 줄어드는 경우가 흔하다.
...
`passive: true`를 주면 브라우저가 스크롤을 바로 처리한다. 특히 모바일 터치에서 체감이 확실하다. Chrome DevTools에서 "Non-passive event listener" 경고 뜨면 바로 고치면 된다.
...
어떤 속성을 바꾸느냐에 따라 세 단계를 다 거칠 수도 있고, 마지막 합성만 거칠 수도 있다.
...
`top`/`left`를 바꾸면 브라우저가 주변 요소 위치를 전부 다시 계산한다. `transform`은 해당 요소만 GPU에서 이동시키니까 다른 요소에 영향이 없다. 보이는 결과는 같은데 비용이 완전히 다르다.
### will-change 선택적 사용
...
`will-change`는 "이 요소가 곧 변할 거야"라고 브라우저한테 알려주는 힌트다. GPU가 미리 레이어를 만들어두는데, 모든 요소에 남발하면 GPU 메모리만 낭비된다. 실제로 움직이는 요소에만, 필요할 때만 쓰자.
