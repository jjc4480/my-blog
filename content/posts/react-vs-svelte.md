...
React 19와 Svelte 5를 네 가지로 나눠서 봤다. 반응성 모델, 컴포넌트 작성 체감, 런타임 성능, 생태계.

## 반응성 모델

상태가 바뀌었을 때 화면을 어떻게 다시 그리는가. React과 Svelte는 정반대의 방식을 택했다.
...
React의 훅은 강력하지만, "올바르게 쓰기"가 어렵다. 의존성 배열 관리, 값이 예전 것으로 잡히는 문제, 불필요한 다시 그리기를 막기 위한 최적화 코드. 이런 것들이 React 코드의 상당 부분을 차지한다. 비즈니스 로직이 아니라 프레임워크를 달래는 코드인 셈이다.

Svelte는 이 문제를 컴파일러에게 넘긴다. 개발자가 의존 관계를 직접 적을 필요가 없으니 실수할 여지도 줄어든다. 대신 "컴파일러가 내 코드를 어떻게 바꾸는지" 이해해야 하는 새로운 학습 포인트가 생긴다.

## 컴포넌트 작성 체감 차이

같은 기능을 만들 때 코드가 얼마나 다른지 보자.
...
Svelte 앱이 다운로드해야 하는 코드량은 React의 1/3~1/5 수준이다. 느린 네트워크나 저사양 폰에서 체감 차이가 크고, 웹 성능 지표에도 유리하다.
...
성능만큼 중요한 게 생태계다. 필요한 라이브러리가 없으면 직접 만들어야 하니까.
...
Svelte를 한 번이라도 써보면 `$state`와 `$derived`의 편리함을 잊기 어렵다. React에서 의존성 배열과 씨름하다가 오면 "원래 이렇게 간단한 거였어?" 싶다. 반대로 React의 명시적인 흐름과 거대한 생태계에 익숙하면 Svelte의 "마법"이 불안할 수도 있다. 결국 프로젝트 상황에 맞는 걸 고르면 된다.
