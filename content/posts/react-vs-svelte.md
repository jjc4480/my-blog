...
프론트엔드 프레임워크를 고를 때 "뭐가 더 좋은가"라는 질문은 의미가 없다. 중요한 건 "이 프로젝트에서 뭐가 더 맞는가"다. React은 2013년부터 쌓아온 생태계가 있고, Svelte는 컴파일러 기반의 접근으로 근본적으로 다른 트레이드오프를 제시한다.

이 글에서는 React 19와 Svelte 5를 네 가지 축으로 비교한다. 반응성 모델, 컴포넌트 작성 체감, 런타임 성능, 그리고 생태계.

## 반응성 모델

프레임워크의 성격을 가장 잘 드러내는 부분이 반응성 모델이다. 상태가 바뀌었을 때 UI를 어떻게 업데이트하는가. React과 Svelte는 이 질문에 정반대의 답을 내놓는다.

### React: 런타임에서 해결한다

React은 상태가 변하면 컴포넌트 함수를 다시 실행한다. 이전 가상 DOM과 새 가상 DOM을 비교(diffing)해서, 실제로 바뀐 부분만 DOM에 반영한다.
...
상태를 선언하려면 `useState`, 파생값을 메모이제이션하려면 `useMemo`, 사이드 이펙트를 처리하려면 `useEffect`. 각각의 훅에 의존성 배열(`[count]`)을 직접 관리해야 한다. 이 배열을 빠뜨리거나 잘못 넣으면 무한 루프가 돌거나 업데이트가 안 된다. React 개발에서 가장 흔한 버그 유형이다.

### Svelte 5: 컴파일러가 해결한다

Svelte는 접근이 다르다. 가상 DOM이 없다. 컴파일러가 빌드 타임에 "어떤 상태가 바뀌면 DOM의 어떤 부분을 업데이트해야 하는지"를 분석해서, 최소한의 업데이트 코드를 생성한다.
...
`$state`로 선언한 변수는 일반 변수처럼 `count++`로 변경할 수 있다. `$derived`는 의존성을 자동으로 추적한다. `$effect`도 마찬가지. 의존성 배열이라는 개념 자체가 없다.
...
React의 훅 시스템은 강력하지만, "올바르게 쓰기"가 어렵다. 의존성 배열 관리, 클로저 함정, 불필요한 리렌더링 방지를 위한 `memo`/`useMemo`/`useCallback` 조합. 이런 것들이 React 코드의 상당 부분을 차지한다. 비즈니스 로직이 아니라 프레임워크를 달래는 코드인 셈이다.

Svelte의 Runes는 이 문제를 컴파일러에게 넘긴다. 개발자가 의존성을 명시할 필요가 없으니 실수할 여지도 줄어든다. 대신 "컴파일러가 무슨 코드를 생성하는지" 이해해야 하는 새로운 학습 비용이 생긴다.
...
**스타일 스코핑.** Svelte는 `<style>` 블록에 쓴 CSS가 해당 컴포넌트에만 자동으로 적용된다. CSS Modules이나 styled-components 같은 별도 설정이 필요 없다. React은 CSS 스코핑 전략을 직접 선택해야 한다.
...
- 훅 규칙(Rules of Hooks)을 이해해야 한다. 조건문이나 반복문 안에서 훅을 호출할 수 없다.
- 상태 불변성(immutability) 개념이 필수다. 객체나 배열을 직접 수정하면 리렌더링이 안 된다.
...
React은 **런타임 프레임워크**다. 브라우저에서 가상 DOM 엔진(reconciler)이 돌아가면서 UI를 관리한다. 이 런타임 코드가 약 42KB. 앱 코드와 별도로 반드시 다운로드해야 한다.

Svelte는 **컴파일러**다. 빌드 타임에 컴포넌트를 바닐라 JavaScript로 변환한다. 프레임워크 런타임이 필요 없다. 브라우저에 전달되는 건 순수한 DOM 조작 코드뿐이다.
...
Svelte 앱의 번들은 React 대비 60~80% 작다. 이 차이는 모바일 네트워크나 저사양 기기에서 체감된다. Core Web Vitals 점수에도 직접적인 영향을 준다.
...
Svelte가 거의 모든 지표에서 앞선다. 가상 DOM diffing이라는 중간 단계가 없기 때문이다. 변경된 상태가 있으면 해당 DOM 노드를 직접 업데이트한다. 비교 연산 자체를 건너뛰는 셈이다.

### 격차는 좁혀지고 있다

React도 가만히 있지 않는다.

- **React Compiler**: React 19에 도입된 자동 최적화. `useMemo`, `useCallback`을 수동으로 넣지 않아도 컴파일러가 불필요한 리렌더링을 잡아준다.
- **React Server Components (RSC)**: 서버에서 렌더링하고 HTML을 스트리밍한다. 클라이언트 번들이 30~50% 줄어든다.

그리고 솔직히, 대부분의 앱에서 프레임워크 성능 차이는 체감하기 어렵다. 네트워크 요청, DB 쿼리, 이미지 로딩이 병목인 경우가 훨씬 많다. "프레임워크 A가 벤치마크에서 30% 빠르다"는 사실이지만, 그 30%가 사용자 경험에 의미 있는 차이를 만드는 경우는 드물다.
...
Next.js는 Vercel과의 통합이 강점이다. 배포, 프리뷰, 에지 런타임이 매끄럽게 연결된다. React Server Components로 클라이언트 번들도 줄일 수 있다. 단, Vercel 외 환경에서는 일부 기능이 제한된다.

SvelteKit은 어댑터 시스템 덕분에 Cloudflare, Vercel, Node, Deno 등 어디에나 배포할 수 있다. 특정 플랫폼에 종속되지 않는다. 번들 크기도 작다. 다만 이미지 최적화나 ISR 같은 기능은 직접 챙겨야 한다.
